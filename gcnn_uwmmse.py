...
class GCL(layers.Layer):
    ...
    def __init__(self, num_filters=3, input_dim=1, output_dim=1, use_bias=True, non_lin=tf.nn.leaky_relu, name="gcl", **kwargs):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            if self.use_bias:
                self.b = tf.compat.v1.get_variable( name='b', shape=(1, self.output_dim), initializer=tf.initializers.glorot_uniform(), dtype=tf.float64, trainable=True)
    ...
    def call(self, S, x):        
        ...
        # LSIGF
        x = tf.reduce_sum( tf.matmul(z,self.w), axis=1 ) #BxNxF_l
    
        # Bias
        if self.use_bias:
            # 1 set of F_l biases for all nodes to make the architecture network-size agnostic
            x = x + self.b #BxNxF_l
