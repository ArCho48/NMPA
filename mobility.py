...
def gauss_markov(nr_nodes, xlim, ylim, t_loc, r_loc, rnge_x,rnge_y, velocity_mean=1., alpha=1., variance=1.):
    '''
    ...
    #if not rnge:
        #rnge_x = int(0.125*MAX_X)
        #rnge_y = int(0.125*MAX_Y)
    #else:
        #rnge_x, rnge_y = rnge
    
    NODES = np.arange(int(0.5*nr_nodes))
    
    #if not t_loc:
        #tx = U(MIN_X, MAX_X, NODES)
        #ty = U(MIN_Y, MAX_Y, NODES)
    #if not r_loc:    
        #rx = tx+U(-rnge_x, rnge_x, NODES)
        #ry = ty+U(-rnge_y, rnge_y, NODES)
    ...
    velocity_t =  np.zeros(int(0.5*nr_nodes))+velocity_mean
    velocity_r =  np.zeros(int(0.5*nr_nodes))+velocity_mean
    theta_t = U(0, 2*np.pi, NODES)
    theta_r = U(0, 2*np.pi, NODES)
    angle_mean_t = theta_t
    angle_mean_r = theta_r
    ...
    while True:
        ...
        tx = tx + velocity_t * np.cos(theta_t)
        ty = ty + velocity_t * np.sin(theta_t)
        rx = rx + velocity_r * np.cos(theta_r)
        ry = ry + velocity_r * np.sin(theta_r)
        
        # node bounces on the margins
        b = np.where(tx<MIN_X)[0]
        tx[b] = 2*MIN_X - tx[b]; theta_t[b] = np.pi-theta_t[b]; angle_mean_t[b] = np.pi-angle_mean_t[b]
        
        b = np.where(tx>MAX_X)[0]
        tx[b] = 2*MAX_X - tx[b]; theta_t[b] = np.pi-theta_t[b]; angle_mean_t[b] = np.pi-angle_mean_t[b]
        
        b = np.where(ty<MIN_Y)[0]
        ty[b] = 2*MIN_Y - ty[b]; theta_t[b] = -theta_t[b]; angle_mean_t[b] = -angle_mean_t[b]
        
        b = np.where(ty>MAX_Y)[0]
        ty[b] = 2*MAX_Y - ty[b]; theta_t[b] = -theta_t[b]; angle_mean_t[b] = -angle_mean_t[b]
        
        b = np.where((rx-tx)<-rnge_x)[0]
        rx[b] = -2*rnge_x - rx[b] + 2*tx[b]; theta_r[b] = np.pi-theta_r[b]; angle_mean_r[b] = np.pi-angle_mean_r[b]
        
        b = np.where((rx-tx)>rnge_x)[0]
        rx[b] = 2*rnge_x - rx[b] + 2*tx[b]; theta_r[b] = np.pi-theta_r[b]; angle_mean_r[b] = np.pi-angle_mean_r[b]
        
        b = np.where((ry-ty)<-rnge_y)[0]
        ry[b] = -2*rnge_y - ry[b] + 2*ty[b]; theta_r[b] = -theta_r[b]; angle_mean_r[b] = -angle_mean_r[b]
        
        b = np.where((ry-ty)>rnge_y)[0]
        ry[b] = 2*rnge_y - ry[b] + 2*ty[b]; theta_r[b] = -theta_r[b]; angle_mean_r[b] = -angle_mean_r[b]
        
        # calculate new speed and direction based on the model
        velocity_t = (alpha * velocity_t +
                    alpha2 * velocity_mean +
                    alpha3 * np.random.normal(0.0, 1.0, int(0.5*nr_nodes)))
    
        theta_t = (alpha * theta_t +
                    alpha2 * angle_mean_t +
                    alpha3 * np.random.normal(0.0, 0.5, int(0.5*nr_nodes)))
        
        velocity_r = (alpha * velocity_r +
                    alpha2 * velocity_mean +
                    alpha3 * np.random.normal(0.0, 1.0, int(0.5*nr_nodes)))
    
        theta_r = (alpha * theta_r +
                    alpha2 * angle_mean_r +
                    alpha3 * np.random.normal(0.0, 0.5, int(0.5*nr_nodes)))
        
        x = np.concatenate([tx,rx],axis=0)
        y = np.concatenate([ty,ry],axis=0)

        speed = np.concatenate([velocity_t,velocity_r],axis=0)
        direction = np.concatenate([theta_t,theta_r],axis=0)
        
        yield np.dstack((x,y))[0], np.dstack((speed,direction))[0]
